...
export type TransactionType = 'sale' | 'expense' | 'cash' | 'withdraw';
...
const AppContent: React.FC = () => {
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-6 text-center">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-800 max-w-md">
          ...
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white mb-4">{t.configRequired}</h1>
          ...
          <div className="space-y-3 text-left">
            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
              ...
              <p className="text-sm font-medium text-slate-800 dark:text-white truncate">{supabaseUrl || t.missing}</p>
            </div>
            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
              <p className="text-xs font-mono text-slate-500">VITE_SUPABASE_ANON_KEY</p>
              <p className="text-sm font-medium text-slate-800 dark:text-white truncate">{supabaseAnonKey ? '••••••••' : t.missing}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  const renderPage = () => {
    switch (activeTab) {
      ...
      case 'withdraw-cash': return <WithdrawCash />;
    ...
    <div className={cn(
      ...
      <main className="lg:ml-64 p-4 lg:p-8 pt-20 lg:pt-8 pb-24 lg:pb-8 overflow-x-hidden">
        ...
        <footer className="max-w-7xl mx-auto w-full mt-12 pt-8 border-t border-slate-200 dark:border-slate-800 text-center">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">{t.developedBy}</p>
